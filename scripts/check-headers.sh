...
if ! jq -e '.headers[] | select(.source == "/api/checkout-session") | .headers[] | select(.key == "Cache-Control" and .value == "no-store, max-age=0")' "$VERCEL_CONFIG" >/dev/null; then
  echo "FAIL: /api/checkout-session cache policy missing or incorrect" >&2
  exit 1
fi
