...
async function fetchJson(url: string): Promise<{ ok: boolean; status: number; data: any; error?: string }> {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      const res = await fetch(url, { headers: { "User-Agent": "sportsync-nhl-movement/1.0" } });
...
async function fetchTargets(
  ...
  return (data ?? [])
    ...
    .filter((id: string) => /_nhl$/i.test(id))
    .map((id: string) => ({ match_id: id, espn_event_id: id.replace(/_nhl$/i, "") }));
}
...
async function resolveMovementItems(eventId: string): Promise<{ providerId: number | null; providerName: string | null; items: any[] }> {
  for (const pid of PROVIDERS) {
    ...
    if (resolved.length > 0) {
      const name = String(resolved[0]?.provider?.name ?? (pid === 100 ? "DraftKings" : pid === 1004 ? "Consensus" : "Bet365"));
